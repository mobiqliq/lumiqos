...
export class ReportCommentGenerator {

    static generateComment(perfLevel: string, subject: string): string {
        if (perfLevel === 'high') {
            return \`Demonstrates exceptionally strong conceptual understanding in \${subject} and participates dynamically mapping high instructional velocity comfortably.\`;
        }
        if (perfLevel === 'medium') {
