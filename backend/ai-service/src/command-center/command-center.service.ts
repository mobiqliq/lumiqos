...
import { CommandCenterAnalyzer } from './command-center.analyzer';
import { TenantContext } from '@lumiqos/shared/index'; // Assume available or mocked context loop

@Injectable()
export class CommandCenterService {
    // In-memory cache mapping ensuring < 1s returns and protecting operational database queries natively.
    private cache: Map<string, { data: any, expiresAt: number }> = new Map();
    private TTL_MS = 10 * 60 * 1000; // 10 minutes

    constructor(
        @InjectRepository(AnalyticsSnapshot) private snapshotRepo: Repository<AnalyticsSnapshot>,
    ...
    private async getActiveYear(schoolId: string): Promise<string> {
        // Simplified dynamic active year resolution (cached in a real env)
        const yr = await this.yrRepo.findOne({ where: { school_id: schoolId, status: 'active' } });
        if (!yr) throw new InternalServerErrorException("No active academic year found natively.");
        return yr.academic_year_id;
    }

    private getCacheKey(schoolId: string, endpoint: string): string {
        return \`command_center:\${schoolId}:\${endpoint}\`;
    }
