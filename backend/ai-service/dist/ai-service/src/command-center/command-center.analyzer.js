...
exports.CommandCenterAnalyzer = void 0;
class CommandCenterAnalyzer {
    static calculateHealthScore(snapshot) {
        ...
        const workloadScore = Math.max(0, 100 - ((snapshot.teacher_workload_index || 0) * 1.5));
        const healthScore = (attendanceScore * 0.25) +
            (homeworkScore * 0.15) +
            (examScore * 0.25) +
            (feeScore * 0.20) +
            (workloadScore * 0.15);
        return Math.floor(Math.min(Math.max(healthScore, 0), 100));
    }
    static generateAlerts(current, previous) {
        const alerts = [];
        const attDrop = previous ? previous.attendance_rate - current.attendance_rate : 0;
        if (attDrop > 18)
            alerts.push({ type: "attendance_drop", severity: "high", message:  } `Attendance dropped \${Math.floor(attDrop)}%\`, affected_area: "Global" });
...
exports.CommandCenterAnalyzer = CommandCenterAnalyzer;
//# sourceMappingURL=command-center.analyzer.js.map