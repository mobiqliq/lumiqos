"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.StudentProfileController = void 0;
const common_1 = require("@nestjs/common");
const student_profile_service_1 = require("./student-profile.service");
const index_1 = require("../../../shared/src/index");
let StudentProfileController = class StudentProfileController {
    constructor(profileService) {
        this.profileService = profileService;
    }
    async triggerBatchRebuild(req) {
        const schoolId = index_1.TenantContext.getStore()?.schoolId || req.user.school_id;
        return this.profileService.triggerBatchRebuild(schoolId);
    }
    async getTeacherProfile(req, studentId) {
        const schoolId = index_1.TenantContext.getStore()?.schoolId || req.user.school_id;
        return this.profileService.getTeacherProfile(schoolId, studentId);
    }
    async getParentInsights(req, studentId) {
        const schoolId = index_1.TenantContext.getStore()?.schoolId || req.user.school_id;
        return this.profileService.getParentInsights(schoolId, studentId);
    }
};
exports.StudentProfileController = StudentProfileController;
__decorate([
    (0, common_1.Post)('ai/student-profile/rebuild'),
    (0, index_1.RequireRoles)('school_owner', 'principal'),
    __param(0, (0, common_1.Req)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], StudentProfileController.prototype, "triggerBatchRebuild", null);
__decorate([
    (0, common_1.Get)('ai/teacher/student-profile/:student_id'),
    (0, index_1.RequireRoles)('teacher', 'school_owner', 'principal'),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Param)('student_id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String]),
    __metadata("design:returntype", Promise)
], StudentProfileController.prototype, "getTeacherProfile", null);
__decorate([
    (0, common_1.Get)('parent/student-insights/:student_id'),
    (0, index_1.RequireRoles)('parent', 'school_owner', 'principal'),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Param)('student_id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String]),
    __metadata("design:returntype", Promise)
], StudentProfileController.prototype, "getParentInsights", null);
exports.StudentProfileController = StudentProfileController = __decorate([
    (0, common_1.Controller)(),
    (0, common_1.UseGuards)(index_1.JwtAuthGuard, index_1.RbacGuard),
    __metadata("design:paramtypes", [student_profile_service_1.StudentProfileService])
], StudentProfileController);
//# sourceMappingURL=student-profile.controller.js.map