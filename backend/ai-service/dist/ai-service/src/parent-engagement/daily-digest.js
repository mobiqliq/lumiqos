...
exports.DailyDigestGenerator = void 0;
class DailyDigestGenerator {
    static synthesize(profile, todayAtt, todayHwList) {
        let attendanceStatus = 'unknown';
        if (todayAtt)
            attendanceStatus = todayAtt.status;
        else if (profile)
            attendanceStatus = 'assumed_present';
        ...
        if (todayHwList) {
            for (const hw of todayHwList) {
                if (hw.status === 'pending')
                    pendingHwCount++;
                else if (hw.status === 'submitted' || hw.status === 'graded')
                    submittedHwCount++;
            }
        }
        const hwString = pendingHwCount > 0 ?  : ;
        `\${pendingHwCount} pending\` : (submittedHwCount > 0 ? 'All completed' : 'None assigned');
...
exports.DailyDigestGenerator = DailyDigestGenerator;
